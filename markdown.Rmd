...
complete_table =
  ...
  transform(complete_table, total_homicide = as.numeric(total_homicide)) %>%
  select(city_state, total_solved, total_unsolved, total_homicide) 
complete_homicidetable
length(complete_homicidetable)
complete_homicidetable %>%
  map2(total_unsolved, total_homicide, prop.test)
  
  mutate(prob = map2(total_unsolved, total_homicide, prop.table),
         pval = map(prob ~ .x[["p.value"]]), 
         CI = map(prob ~ as.numeric(.x[["conf.int"]])))
